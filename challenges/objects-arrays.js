...
//   }
//   weightFun() {
//     console.log(`${this.name} weighed ${this.weight}`);
//   }
//   dietFun() {
//     console.log(`${this.name} had a deit of ${this.diet}`);
//   }
//   lengthFun() {
//     console.log(`${this.name} was ${this.length}!!`);
//   }
//   timeFun() {
//     console.log(`${this.name} was apart of the ${this.period} era!!`);
//   }
// }
...
// tyrannosaurus.prototype.roar = () => `RAWERSRARARWERSARARARRRR!`;
...
function universitiesPush(arr) {
  for (let i = 0; i < arr.length; i++) {
    unsortedUniversities.push(arr[i].university);
  }
}
universitiesPush(graduates);
// console.log(unsortedUniversities);
...
let contactInfo = [];
function contactEmail(arr) {
  for (let i = 0; i < arr.length; i++) {
    contactInfo.push(arr[i].first_name + " " + arr[i].email);
  }
}
contactEmail(graduates);
//! console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
let unisWithUni = [];

function unisWithUnis(arr) {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].university.includes("Uni")) unisWithUni.push(arr[i].university);
  }
}
unisWithUnis(graduates);
...
displayNamesForEach = () =>
  zooAnimals.forEach(arr =>
    displayNames.push(arr.animal_name + ", " + arr.scientific_name)
  );

displayNamesForEach(zooAnimals);
...
let lowCaseAnimalNames = zooAnimals.map(
  arr => `${arr.animal_name.toLowerCase()}`
);
...
*/
let lowPopulationAnimals = zooAnimals.filter(arr => arr.population <= 5);
...
let populationTotal = zooAnimals.reduce(
  (accumulator, arr) => accumulator + arr.population,
  0
);
